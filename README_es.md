# Nmap Descubrimiento de hosts y escaneo de puertos

**Idiomas:** [English](README.md) · [简体中文](README_zh.md) · [Español](README_es.md) · [Français](README_fr.md) · [Deutsch](README_de.md) · [日本語](README_ja.md) · [Русский](README_ru.md) · [한국어](README_ko.md) · [Português](README_pt.md)

<p align="center">
  <a href="https://labex.io/es/courses/mastering-nmap-host-discovery-port-scanning">
    <img src="https://course-cover.labex.io/mastering-nmap-host-discovery-port-scanning.png?lang=es" alt="Nmap Descubrimiento de hosts y escaneo de puertos">
  </a>
</p>

Domina Nmap, el estándar de la industria para el mapeo de redes. Aprende técnicas de descubrimiento de hosts sigilosas, ejecuta diversos tipos de escaneo TCP/UDP y optimiza el rendimiento para entornos empresariales a gran escala.

[Iniciar curso en LabEx](https://labex.io/es/courses/mastering-nmap-host-discovery-port-scanning)

## Ejercicios

|   Índice | Nombre                                  | Dificultad   | Práctica                                                                                                                                                         |
|----------|-----------------------------------------|--------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|        1 | Estrategias de descubrimiento de hosts  | Principiante | <a target='_blank' href='https://labex.io/es/labs/linux-host-discovery-strategies-656135?course=mastering-nmap-host-discovery-port-scanning'>Iniciar lab</a>     |
|        2 | Escaneo de puertos TCP y UDP            | Principiante | <a target='_blank' href='https://labex.io/es/labs/linux-tcp-and-udp-port-scanning-656138?course=mastering-nmap-host-discovery-port-scanning'>Iniciar lab</a>     |
|        3 | Optimización del rendimiento de escaneo | Principiante | <a target='_blank' href='https://labex.io/es/labs/linux-optimizing-scan-performance-656137?course=mastering-nmap-host-discovery-port-scanning'>Iniciar lab</a>   |
|        4 | Formatos de salida de Nmap              | Principiante | <a target='_blank' href='https://labex.io/es/labs/linux-nmap-output-formats-656136?course=mastering-nmap-host-discovery-port-scanning'>Iniciar lab</a>           |
|        5 | Desafío de Mapeo de Red                 | Intermedio   | <a target='_blank' href='https://labex.io/es/labs/linux-network-mapping-challenge-656134?course=mastering-nmap-host-discovery-port-scanning'>Iniciar desafío</a> |

## About LabEx

<div align="left"><p><a href="https://labex.io"><strong>LabEx</strong></a> is a <strong>hands-on learning platform for beginners</strong>.</p><p>Explore <a href="https://labex.io/learn/linux"><strong>Linux</strong></a>, <a href="https://labex.io/learn/devops"><strong>DevOps</strong></a>, <a href="https://labex.io/learn/cybersecurity"><strong>Cybersecurity</strong></a>, and <strong>more</strong> — all directly in your browser.</p><p>Learn step by step through <strong>interactive labs</strong>, <strong>guided exercises</strong>, and <strong>real-world projects</strong>. 🌱<br />No setup, no stress — just practice and grow your skills by doing.</p><br /><p><a href="https://apps.apple.com/app/id6765840991"><img src="https://developer.apple.com/assets/elements/badges/download-on-the-app-store.svg" alt="Download on the App Store" height="54" /></a>&nbsp;<a href="https://apps.apple.com/app/id6765840991"><img src="https://developer.apple.com/app-store/marketing/guidelines/images/badge-download-on-the-mac-app-store.svg" alt="Download on the Mac App Store" height="52" /></a></p><br /><p>📖 Need help? Visit our <a href="https://support.labex.io/">Help Center</a> or email info@labex.io</p></div>

